...
same sockets your own formats and transports use, and it earns its standing
...
from . import code, formats, readers, reasoning, tools
...
def install(registry=None, *, exist_ok: bool = True) -> None:
    ...
    reasoning.install(registry, exist_ok=exist_ok)
    readers.install(registry, exist_ok=exist_ok)
    tools.install(registry, exist_ok=exist_ok)
    code.install(registry, exist_ok=exist_ok)


__all__ = ["VERSION", "code", "formats", "install", "readers", "reasoning", "tools"]
