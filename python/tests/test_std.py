"""The std pack: formats json/table/scaled_number, reader json_object,
reasoning transports — through the sockets, against the spec."""
...
import lmcc
import lmcc_std
from lmcc_std.readers import JsonObjectReader
...
def test_json_format_lowers_and_lifts_native_values():
    ...
    req = plan.render(text="t", turns=[plan.example({"text": "d"}, {"people": [Person("Ann", 41)], "color": Color.RED, "n": 1})])
...
def test_table_format_escaping_and_coercion():
    ...
    demo = plan.render(text="t", turns=[plan.example({"text": "d"}, {"rows": [{"name": "a|b", "ok": True}]})]).messages[1]["parts"][0]["text"]
    assert demo == "<rows>\n| a\\|b | true |\n</rows>"
...
def test_json_object_reader_is_a_gated_mode():
    ...
    req = plan.render(q="?")
    assert req.request_settings["config"]["response_format"]["schema"]["required"] == ["answer", "score"]
...
def test_json_object_join_embed_rule_is_raw_text_identity():
    reader = JsonObjectReader({"kind": "json_object"})
    spelled = [("s", "plain"), ("n", "9"), ("b", "true"), ("quoted", '"hi"'), ("arr", "[1, 2]")]
    raw = reader.split(reader.join(spelled), [n for n, _ in spelled])
    assert raw == {"s": "plain", "n": "9", "b": "true", "quoted": '"hi"', "arr": "[\n    1,\n    2\n  ]"}


def test_reasoning_transports_by_name():
    ...
    adp = lmcc.adapter(messages=[lmcc.system("{% for f in outputs %}<{f.name}>\n{f.value}\n</{f.name}>\n{% endfor %}"),
                                 lmcc.user("{q}")], transports={"reasoning": "reasoning_tags"})
