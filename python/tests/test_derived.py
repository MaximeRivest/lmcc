...
import lmcc
from lmcc.strategy import eval_predicate

PATTERN_TEMPLATE = lmcc.template([
...
def _baked(template=PATTERN_TEMPLATE):
    return lmcc.adapter(template=template,
...
def test_indistinct_anchors_refuse_at_bake():
    template = lmcc.template([
        lmcc.message("system", "{% for f in outputs %}Value: {f.value}\n"
                    "{% endfor %}"),
        lmcc.message("user", "{q}"),
    ])
    with pytest.raises(lmcc.LMCCError) as err:
...
def test_no_pattern_block_refuses():
    template = lmcc.template([lmcc.message("user", "{q}")])
    with pytest.raises(lmcc.LMCCError) as err:
        _baked(template)
    assert err.value.code == "not-lensable"
...
def test_derived_lens_tracks_hidden_fields():
    ...
    sig = lmcc.signature(
        ...
        outputs={"reasoning": lmcc.field(str, role="reasoning"),
                 "answer": str})
    adp = lmcc.adapter(template=PATTERN_TEMPLATE.copy()
...
def test_predicate_gates_bake():
    ...
    adp = lmcc.adapter(
        template=lmcc.template([lmcc.message("system", "{instruction}"),
                               lmcc.message("user", "{q}")]),
...
def test_predicate_roundtrips_in_the_artifact():
    ...
    adp = lmcc.adapter(
        ...
        parse={"kind": "sections", "open": "<{name}>"},
        strategies={"reasoning": strategy})
    entry = lmcc.dump(adp, lmcc.Registry())
    loaded = lmcc.load(entry, registry=lmcc.Registry())
...
def test_malformed_predicate_refuses():
    with pytest.raises(lmcc.LMCCError) as err:
        lmcc.Strategy.from_dict({"predicate": {"nope": 1}}, where="test")
