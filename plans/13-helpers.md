...
**Status: proposal, 2026-09-23. Nothing built. Needs the maintainer's
yes on the names before code.**
...
- [ ] `python/lmcc/helpers.py`: each helper returns the dict above; a
      wrong argument raises `TypeError` in Python (a host misuse, not a
      `Refusal`, like `Fn.__call__`).
- [ ] `tests/test_helpers.py`: every helper equals its hand-written dict,
      and a transport built both ways dumps to the same entry.
- [ ] GUIDE §7 and how-tos 03, 11, 12 use the helpers, showing the dict
      once so a reader sees it is data.
- [ ] Glossary rows; `./check` green.
