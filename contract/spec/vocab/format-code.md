...
`accepts: object` · `direction: both` · `writes: text` · `reads: text` ·
`round_trip: true`. Value: exactly `{"code": string}`. Writes the code
verbatim; reads raw text parts without `Capture.text` stripping. Empty code,
...
`accepts: list[*], *` · `direction: both` · `writes: parts` ·
...
capture reads `[]`; an empty captured body reads one call with empty code.
...
`tool_catalog`. Text find rule captures between `"run_python <<'PY_END'\n"`
and `"\nPY_END"` (substitute options), removes it, targets `@purpose.calls`,
and `complete_reply: true`. The opening/closing delimiters are plain core scans,
not regex and not a shell interpreter. The transport, call template, and
probe are derived from the same options.

`tell` text, system (with option substitution):
...
another tool; the writer of past calls itself only formats arguments and
inserts `{name}`. The caller must only replay calls from this transport
through it; the bind-time sample check is not per-call validation.
