...
type JSONCodec struct{ indent int }

func newJSONCodec(options *lmcc.Object) (lmcc.Codec, error) {
	c := &JSONCodec{indent: 2}
...
func (c *JSONCodec) RenderSchema(shape *lmcc.Object) string {
	return "JSON matching this schema: " + lmcc.MarshalJSON(shape, -1)
}

func (c *JSONCodec) RenderValue(value any, shape *lmcc.Object) (string, error) {
	return lmcc.MarshalJSON(value, c.indent), nil
}

func (c *JSONCodec) ParseValue(text string, shape *lmcc.Object) (any, error) {
...
func newTableCodec(options *lmcc.Object) (lmcc.Codec, error) {
	cols := options.List("columns")
	if cols == nil {
		return nil, fmt.Errorf("table codec requires the 'columns' option")
	}
	c := &TableCodec{delimiter: "|", escape: "\\", null: ""}
...
func (c *TableCodec) row(cells []string) string {
	d := c.delimiter
	return d + " " + strings.Join(cells, " "+d+" ") + " " + d
}

func (c *TableCodec) RenderSchema(shape *lmcc.Object) string {
	return c.row(c.columns) + "  (one row per item)"
}

func (c *TableCodec) RenderValue(value any, shape *lmcc.Object) (string, error) {
	items, ok := value.([]any)
	if !ok {
...
func (c *TableCodec) spell(v any, col string) (string, error) {
	switch x := v.(type) {
	case nil:
		return c.null, nil
	case string:
...
func (c *TableCodec) ParseValue(text string, shape *lmcc.Object) (any, error) {
	var props *lmcc.Object
	if items := shape.Object("items"); items != nil {
...
func (c *TableCodec) isHeader(cells []string) bool {
	if len(cells) != len(c.columns) {
		return false
	}
...
func (c *TableCodec) split(line string) []string {
...
type ScaledNumberCodec struct {
	scale  float64
	suffix string
	round  *int
}

func newScaledNumberCodec(options *lmcc.Object) (lmcc.Codec, error) {
	c := &ScaledNumberCodec{scale: 1}
...
func (c *ScaledNumberCodec) RenderSchema(shape *lmcc.Object) string {
	example := "0.83"
	if c.scale == 100 {
		example = "83"
	}
...
func (c *ScaledNumberCodec) RenderValue(value any, shape *lmcc.Object) (string, error) {
	var f float64
	switch x := value.(type) {
	case int64:
		f = float64(x)
...
func (c *ScaledNumberCodec) ParseValue(text string, shape *lmcc.Object) (any, error) {
	t := lmcc.Strip(text)
	if c.suffix != "" {
