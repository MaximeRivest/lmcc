...
3. **Scalars are mechanics; structure is vocabulary.** The kernel spells
   ...
   refuses.
4. **Strategies are data with a predicate.** Capability facts are declared,
   checked at bake, and refused by name — before any money is spent.
5. **Loud refusal, stable codes.** `contract/spec/errors.md`; asserted by
   the corpus. Silent wrong rendering is the one forbidden bug.
6. **The corpus is the authority.** A second implementation (Go, TS) is
