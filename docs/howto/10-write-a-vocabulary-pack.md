...
**Goal.** Register a named format and a named transport through the
...
class CsvFormat(lmcc.Format):
    ...
    def read(self, capture, field):
        return [p.strip(ASCII_WS) for p in capture.text.split(self.sep)] if capture.text else []

def csv_factory(options):
    return CsvFormat(options)
```

## 2. A transport factory

A named transport is a factory `options -> Transport`. The kernel checks
...
def scratchpad(options):
    ...
    return lmcc.Transport(
        requires=["instruct"],
        tell={"system": f"Write your notes on lines starting with {prefix!r}."},
        find=[{"from": "text", "line_prefixed": prefix, "to": "@purpose", "remove": True}],
        in_template=False)
```
...
def install(registry, *, exist_ok=True):
    ...
    registry.register_transport("scratchpad", scratchpad, version="0.1.0", exist_ok=exist_ok)
...
case = {
    ...
    "entry": {
        ...
        "template": [
            ...
            {"role": "user", "text": "{text}"}],
        "reader": {"kind": "derived"},
        "transports": {"reasoning": {"use": "scratchpad", "options": {"prefix": "NOTE:"}}},
...
try:
    ...
    assert r.code == "unknown-transport"
    assert r.fix == {"action": "install-vocabulary", "kind": "transport", "name": "scratchpad"}
...
| `unknown-format`, `unknown-transport` | at load or dump: the registry lacks the name |
